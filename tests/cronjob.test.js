...
const { getProcessQueue } = require("../processQueue");
const Server = require("../middleware/index.js");
const Router = require("../routes/index.js");
const Group = require("../models/group");
const PaymentModel = require("../models/payment");
...
describe("Cronjob API Endpoints", () => {
  let user, authToken, service, group, message, category, app;
  ...
  beforeEach(async () => {
    ...
    authToken = await createAuthToken(user);
    message = await createMessage(group, user, [user]);
  });
  ...
  describe("GET /recurring-invoices", () => {
    ...
    it("should send recurring invoices to group members", async () => {
      ...
      newGroup = await Group.findById(newGroup._id);
      const newDate = newGroup.nextSubscriptionDate;
      expect(newDate.getTime()).toBeCloseTo(expectedDate.getTime(), -3);
      expect(getProcessQueue).toHaveBeenCalled();
      expect(getProcessQueue().add).toHaveBeenCalled();
    }, 10000);
  });
  ...
  describe("GET /check-members-payments", () => {
    it("should remove inactive members from groups and send notifications", async () => {
      ...
      group.members.push({
        ...
        subscriptionStatus: "active",
        paymentActive: false,
        lastInvoiceSentAt: new Date(Date.now() - 49 * 60 * 60 * 1000),
      });
      await group.save();
  ...
  describe("GET /verify-disbursements", () => {
    it("should verify pending disbursements", async () => {
      ...
      await delay(1000);
      const verifiedPayment = await PaymentModel.findById(payment._id);
      expect(verifiedPayment.disbursed).toBe("successful");
    });
  });
});
